# Navigation Scenario Adapted for truly shared rewards and working communication ranges
...
class Scenario(BaseScenario):
    def make_world(self, batch_dim: int, device: torch.device, **kwargs):
        ...
        self.comms_range = kwargs.pop("comms_range", 1)
        self.shared_rew = kwargs.pop("shared_rew", True)
        self.n_agents = kwargs.pop("n_agents", 4)

        self.pos_shaping_factor = kwargs.pop("pos_shaping_factor", 4)  # max is 8
        self.final_reward = kwargs.pop("final_reward", 10)

        self.agent_collision_penalty = kwargs.pop("agent_collision_penalty", -0.1)
        self.passage_collision_penalty = kwargs.pop("passage_collision_penalty", -0.1)
        ...
        for i in range(self.n_agents):
            ...
            agent.passage_collision_rew = agent.pos_rew.clone()
    ...
    def reward(self, agent: Agent):
        ...
        if not self.shared_rew:
            agent.agent_collision_rew[:] = 0
            agent.passage_collision_rew[:] = 0  # reset passage penalty
            ...
            # Passage collision penalty
            for wall in self.long_walls + self.short_walls:
                dist = self.world.get_distance(agent, wall)
                collision_mask = dist <= self.min_collision_distance
                agent.passage_collision_rew[collision_mask] += self.passage_collision_penalty


            return (
                    (self.pos_rew if self.shared_rew else agent.pos_rew)
                    + agent.agent_collision_rew
                    + agent.passage_collision_rew
                    + self.final_rew
            )

        else:
            total_agent_collision_penalty = torch.zeros_like(self.pos_rew)
            total_passage_collision_penalty = torch.zeros_like(self.pos_rew)

            # Boolean mask used for performance
            for a in self.world.agents:
                for b in self.world.agents:
                    if a != b:
                        collision = self.world.get_distance(a, b) <= self.min_collision_distance
                        total_agent_collision_penalty += collision * self.agent_collision_penalty

                for wall in self.long_walls + self.short_walls:
                    collision = self.world.get_distance(a, wall) <= self.min_collision_distance
                    total_passage_collision_penalty += collision * self.passage_collision_penalty

            return (
                    self.pos_rew
                    + total_agent_collision_penalty
                    + total_passage_collision_penalty
                    + self.final_rew
            )
    ...
    def observation(self, agent: Agent):
        observations = [
            ...
            torch.linalg.vector_norm(agent.state.pos - agent.goal.state.pos, dim=-1).unsqueeze(-1),
        ]

        # New part: relative positions of nearby agents
        for other in self.world.agents:
            if other is not agent:  # Use 'is not' for object identity comparison
                # Calculate relative position between agent and other for all envs in batch
                relative_pos = other.state.pos - agent.state.pos

                # Calculate distance for all environments in the batch
                dist = torch.linalg.vector_norm(relative_pos, dim=-1)  # Shape (batch_dim,)

                # Create a boolean mask: True if within range, False otherwise
                in_range_mask = dist <= self.comms_range

                # Expand mask to match the dimensions of relative_pos (batch_dim, 2)
                in_range_mask_expanded = in_range_mask.unsqueeze(-1)

                # If in_range_mask_expanded is True, take relative_pos, else take zeros
                masked_relative_pos = torch.where(
                    in_range_mask_expanded,
                    relative_pos,
                    torch.zeros_like(relative_pos)
                )
                observations.append(masked_relative_pos)
    ...
    def info(self, agent: Agent):
        return {
            ...
            "agent_passage_collision_rew": agent.passage_collision_rew,
